...
export function LotColumn({
    ...
    return (
        <div
            className={cn(
                "flex-1 min-w-[280px] max-w-[320px] shrink-0 lg:shrink flex flex-col h-full max-h-full bg-muted/20 rounded-3xl border border-border/40 p-3 transition-all",
                isDragOver ? "scale-[1.02] ring-2 ring-primary/20 shadow-xl bg-primary/5" : "hover:bg-muted/30"
            )}
            onDragOver={onDragOver}
            onDrop={(e) => onDrop(e, status.id)}
        >
            <div className="flex items-center justify-between px-3 pb-3 pt-2 mb-2 border-b border-border/40 shrink-0">
            ...
            <div className="flex flex-col gap-3 overflow-y-auto overflow-x-hidden flex-1 p-1 min-h-0 custom-scrollbar">
                <AnimatePresence>
                    {lots.map(lot => (
                        <LotCard
                            key={lot.id}
